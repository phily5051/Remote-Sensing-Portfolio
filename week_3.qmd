...
<h3>3.1.2 Data Corrections</h3>

Geometric, Atmospheric and Orthorectification corrections can be applied to corrected distorted images.

<h4>**(1)** **Geometric Correction**</h4>
...
<h4>**(2)** **Atmospheric Correction**</h4>
...
<h4>**(3)** **Orthorectification Correction**</h4>
...
<h3>3.2.1 An Image Processing Technique for the Study of Urban Heat Island Changes Using Different Seasonal Remote Sensing Data</h3>

-   **Summary**: @Hanqiu2011AnIP showed a method to address the challenge of comparing thermal remote sensing imagery data from different dates when studying UHI phenomena. The method involves using image enhancement technologies to generate 3D images that highlight UHI heat characteristics and spatial distribution features. To reduce the impacts caused by seasonal differences or weather conditions, normalization and scaling are used. Then, the images were overlaid to show difference between images by subtracting corresponding pixels. The study also introduces the Urban Heat Island Ratio Index (URI) to quantitatively compare UHI intensity within urban areas.

-   **Comment**: The study focuses on the difficulty of comparing thermal remote sensing imagery data from different dates due to seasonal variations and cloudy/rainy conditions, which is crucial to studying urban heat island (UHI) phenomena. The proposed solution involves processing the thermal data from different dates separately, using image enhancement technologies to generate 3D images, and reducing seasonal differences using normalization and scaling. The study also introduces a quantitative measure, the Urban Heat Island Ratio Index (URI), to compare UHI intensity within urban areas. However, more information is needed on the specific image enhancement technologies used and how they were selected. Additionally, I think it would be valuable to test this method in locations with different UHI patterns and environmental conditions beyond the case study city presented in the paper.

\<h3\>3.2.2 RSCNN: A CNN-Based Method to Enhance Low-Light Remote-Sensing Images\</h3\>
...
-   **Comment**: In addition to image enhancement technologies covered in the lecture, this study demonstrates that Convolutional Neural Networks (CNN) can enhance visual appearance of image greatly. The study's proposed RSCNN architecture is an exciting development in image enhancement technology, particularly for remote-sensing images. The use of real natural image patches for initial training and fine-tuning with simulated remote-sensing images is a smart approach to addressing the lack of labeled training data. The experiments' qualitative and quantative results demonstrate RSCNN's superiority over conventional techniques, which is encouraging. Lastly, I would like to point that this research somehow offered potentials in combining two different fields of studies - CNN and remote sensing -, and suggested that adding complexity to imagery can help us more natural results with more realistic textures and vivid details.
